...
**Homogeneous** IFS are those where the functions all share the same linear part: some famous examples are *wild* and *tame* $k$-*dragons*, *Sierpi&#324;ski* $k$-*gons*, and *Bedford-McMullen carpets*. **Non-homogeneous** IFS are those where different scaling factors are allowed, like the *L&eacute;vy*, *Koch*, and *Kiesswetter* curves.
...
Fix $N\geq2$ and let $\mathcal{T}:=\{\lambda\in\mathbb{D}~:~A\text{ is p.c.f. and of bounded turning}\}$. Let $\sigma$ be a permutation on $\{0,1,\cdots,N-1\}$ and consider the IFS of $N$ functions defined as $F_j(z):=f_j\left(\zeta_N^{\sigma(j)}z\right)$ For every $\lambda\in\mathcal{T}$, there exists $\sigma$ such that the function $q:A\to A$, where $q|_{f_j(A)}:=F_j^{-1}$, is well defined. Moreover, the dynamics of $q$ is quasisymmetrically conjugate to that of $p$ on its Julia set, where $p(z)=z^n+c/z^m$ for some particular choice of $n\geq2$ and $m\geq0$ with $n+m=N$ and $c$ is a Misiurewicz parameter. Moreover, if $\lambda\in\mathcal{T}\cap\mathbb{R}$ then $c\in\mathbb{R}$. {data-type=conjecture aria-label=conjecture} 

I am focusing on determining explicitly the relation of the tuple $(n,m)$ with the permutation $\sigma$.
